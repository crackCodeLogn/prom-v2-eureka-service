APP_NAME="prom-v2-eureka-service"
APP_VERSION="0.0.1-SNAPSHOT"
JAVA_PARAM="-Xmx251m"
APP_PARAMS="-DSPRING_SECURITY_USERNAME=$PROM_SPRING_USER -DSPRING_SECURITY_CRED=$PROM_SPRING_CRED -DEUREKA_USER_NAME=$PROM_EUREKA_USER -DEUREKA_USER_CRED=$PROM_EUREKA_CRED"

BIN_PATH=$PROM_HOME_PARENT/MORP/$APP_NAME/bin     #PROM-HOME-PARENT :: exported in .bashrc
JAR_PATH=$BIN_PATH/../target/$APP_NAME-$APP_VERSION.jar

echo "Starting '$APP_NAME' with java param: '$JAVA_PARAM', app params: '$APP_PARAMS' at '$JAR_PATH'"
java $JAVA_PARAM $APP_PARAMS -jar $JAR_PATH
